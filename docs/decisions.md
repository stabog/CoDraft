...
- **Shift+выделение** — расширяет существующий якорь (объединение диапазонов); работает с кистью и без неё, если якорь уже есть.
- **Сброс якоря:** Escape; кнопка «Снять»; новое выделение с кистью/Alt (без Shift) заменяет якорь. Клик вне якоря перемещает курсор, якорь сохраняется (для расширения по Shift).
...
- **Определение тона:** отдельный LLM-запрос («Определить тон») заполняет `initialToneAxes`; до этого baseline = 5. Целевой тон — колесо/слайдеры.
